...
int DBFile::Create (char *f_path, fType f_type, void *startup) {
	switch(f_type)
	{
		...
		case tree:break;
		default:
			std::cout << "unknow file type in creating \"" << f_path << "\"\n";
			return 0;
	}
	return 1;	
}

//this function should only be called only once, and also should be called
//at the start of all other functions (of course should after create and open
//command), so implicitly, we assume the user has make the "tool" connected
//to a disk file 
